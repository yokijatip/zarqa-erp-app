...
fun SteamScreen(
    ...
    viewModel: SteamViewModel = viewModel()
) {
                ...
                if (!isLoading && batches.isEmpty()) {
                    item {
                        Box(
                            ...
                            Text(
                                text = "Tidak ada batch untuk di-steam",
                    ...
                    items(batches, key = { it.id }) { batch ->
                        val isAntri = batch.status == "JAHIT_DONE"
                        BatchCard(
                            batch = batch,
                            primaryLabel = if (isAntri) "Mulai Steam" else "Selesai & Input",
                            onPrimaryAction = {
                                if (isAntri) {
                                    batchToStart = batch
                                } else {
    ...
    batchToStart?.let { batch ->
        AlertDialog(
            ...
            title = { Text("Mulai Steam") },
            text = { Text("Mulai proses steam untuk ${batch.nama_model}?") },
            confirmButton = {
                TextButton(onClick = {
                    batchToStart = null
                    viewModel.startSteam(batch.id, userProfile.uid, userProfile.name) { success ->
                        scope.launch {
                            snackbarHostState.showSnackbar(
                                if (success) "Steam dimulai!" else "Gagal memulai. Coba lagi."
                            )
                        }
                    }
                }) { Text("Mulai") }
    ...
    if (batchToFinish != null) {
        ModalBottomSheet(
            ...
            Column(modifier = Modifier.padding(horizontal = 24.dp, vertical = 8.dp)) {
                Text("Laporan Hasil Steam", style = MaterialTheme.typography.titleMedium)
                Text(
                    ...
                    androidx.compose.material3.Button(onClick = {
                        ...
                        when {
                            ...
                            pcs + reject > batch.total_pcs ->
                                ...
                                batchToFinish = null
                                viewModel.finishSteam(batchId, userProfile.uid, userProfile.name, pcs, reject, catatan.ifBlank { null }) { success ->
                                    scope.launch { snackbarHostState.showSnackbar(if (success) "Steam selesai!" else "Gagal menyimpan. Coba lagi.") }
                                }
                            }
                        }
                    }) { Text("Simpan & Selesai") }
